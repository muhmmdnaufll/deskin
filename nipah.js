(() => {
  ...
  const defaultLocations = [
    { id: "lae-singkil-1", name: "Kawasan Pesisir Lae Singkil", note: "Observasi awal potensi tegakan nipah dan akses masyarakat pesisir.", x: 38, y: 48, status: "Observasi awal" },
    { id: "muara-1", name: "Muara Estuari", note: "Area potensial untuk edukasi konservasi dan hilirisasi nira.", x: 58, y: 35, status: "Perlu validasi" },
    { id: "gampong-1", name: "Calon Titik Kolaborasi Desa", note: "Rencana diskusi dengan penyadap, BUMDes, dan kelompok tani.", x: 68, y: 66, status: "Rencana" }
  ];
  ...
  const articleSeed = [
    {
      ...
      category: "Lingkungan",
      minutes: 4,
      body: "Nipah membantu menjaga kawasan estuari, memberi ruang hidup bagi biota pesisir, dan bisa menjadi sumber ekonomi tanpa harus merusak tegakan hidupnya. Nilai pentingnya bukan hanya pada nira, tetapi pada fungsi ekologis yang melindungi pesisir."
    },
    {
      title: "Nira Cepat Rusak, Kenapa Perlu Hilirisasi?",
      category: "Ekonomi Lokal",
      minutes: 5,
      body: "Nira nipah mudah berubah kualitas setelah disadap. Karena itu, pencatatan waktu, wadah higienis, dan pengolahan awal menjadi penting. Hilirisasi membuat nira tidak berhenti sebagai bahan mentah bernilai rendah."
    },
    {
      title: "Apa Itu Biostimulan Cair Nipah?",
      category: "Pertanian",
      minutes: 5,
      body: "Biostimulan Cair Nipah adalah gagasan pengolahan nira nipah melalui fermentasi terkendali untuk mendukung pertumbuhan tanaman. Pada tahap awal, produk ini diposisikan sebagai prototipe uji terbatas, bukan pengganti total pupuk."
    },
    {
      title: "Energi Baik dalam Aksi Lingkungan",
      category: "Sobat Bumi",
      minutes: 3,
      body: "Energi baik bisa berbentuk pengetahuan, kolaborasi, kepedulian, dan keberanian memulai aksi. Dalam Nipah Lestari, energi baik disebarkan melalui observasi lapangan, edukasi digital, dan ajakan menjaga pesisir."
    }
  ];

  const impactCards = [
    { title: "Lingkungan", value: "Tegakan dijaga", text: "Pemanfaatan diarahkan tanpa menebang nipah, sehingga nilai ekonomi mendukung konservasi." },
    { title: "Sosial", value: "Kolaborasi lokal", text: "Penyadap, BUMDes, petani, mahasiswa, dan dinas dapat terhubung dalam satu gerakan awal." },
  ...
  function bindGlobalEvents() {
    ...
    $("#themeBtn").addEventListener("click", () => {
      ...
      document.documentElement.dataset.theme = state.theme;
      saveState();
      showToast(`Tema ${state.theme === "dark" ? "gelap" : "terang"} aktif.`);
    });
  }

  function renderNav() {
    navRoot.innerHTML = navItems.map((navItem, navIndex) => `
      <a href="${navItem.path}" data-link>
        <span class="nav-number">${String(navIndex + 1).padStart(2, "0")}</span>
    ...
    app.innerHTML = `
      <section class="container stack">
        <div class="hero">
          ...
          <div class="leaf-card">
            <div class="between"><strong>Nipah Lestari</strong><span class="pill good">Sobat Bumi 2026</span></div>
      ...
      <section class="container stack">
        ...
        <div class="grid two">
          ...
          <section class="card stack">
            ...
            <div id="fieldNoteList" class="stack">${fieldNotesMarkup()}</div>
          </section>
        </div>
      ...
      <section class="container stack">
        <div class="hero">
          ...
          <div class="card stack">
            <span class="pill good">Fokus Aceh Singkil</span>
            <h2>768,08 ha</h2>
  ...
  function renderDampak() {
    ...
    app.innerHTML = `
      <section class="container stack">
        ...
        <div class="grid four">${impactCards.map(impactCard).join("")}</div>
        <section class="card stack">
  ...
  function renderEdukasi() {
    ...
    app.innerHTML = `
      <section class="container stack">
        ...
        <div class="grid two">${articleSeed.map(articleCard).join("")}</div>
      </section>
    `;
    bindArticleCards();
    bindAiButtons();
  ...
  function statCard(value, label, text) {
    return `<article class="card stat"><span class="tag">${escapeHtml(label)}</span><strong>${escapeHtml(String(value))}</strong><small class="muted">${escapeHtml(text)}</small></article>`;
  }

  function featureCard(icon, title, text) {
    return `<article class="feature-card"><div class="feature-icon">${icon}</div><h3>${escapeHtml(title)}</h3><p class="muted">${escapeHtml(text)}</p></article>`;
  }

  function timelineStep(number, title, text) {
    return `<div class="timeline-step"><span>${number}</span><div><h3>${escapeHtml(title)}</h3><p>${escapeHtml(text)}</p></div></div>`;
  }

  function impactCard(impact) {
    return `<article class="impact-card"><span class="tag">${escapeHtml(impact.title)}</span><h3>${escapeHtml(impact.value)}</h3><p class="muted">${escapeHtml(impact.text)}</p></article>`;
  }

  function articleCard(article, index) {
    return `<article class="article-card" data-article="${index}"><span class="tag">${escapeHtml(article.category)} · ${article.minutes} menit</span><h3>${escapeHtml(article.title)}</h3><p class="muted">${escapeHtml(article.body.slice(0, 130))}...</p></article>`;
  }

  function mapMarkup() {
    return `<div class="map-box"><div class="map-river"></div>${state.locations.map((locationPoint) => `
      <div class="map-pin" style="--x:${locationPoint.x};--y:${locationPoint.y}" title="${escapeHtml(locationPoint.name)}">
        <i></i><small>${escapeHtml(locationPoint.status)}</small>
      </div>
    `).join("")}</div>`;
  }
  ...
  function fieldNotesMarkup() {
    ...
    return state.fieldNotes.map((fieldNote) => `
      <article class="note-card">
        <span class="tag">${escapeHtml(fieldNote.date)}</span>
        <h3>${escapeHtml(fieldNote.title)}</h3>
        <p class="muted">${escapeHtml(fieldNote.note)}</p>
      </article>
    `).join("");
  }
  ...
  function supportersMarkup() {
    ...
    return state.supporters.slice(0, 6).map((supporter) => `
      <article class="note-card">
        <span class="tag">${escapeHtml(supporter.role)}</span>
        <h3>${escapeHtml(supporter.name)}</h3>
        <p class="muted">${escapeHtml(supporter.origin || "Tanpa asal")} · ${escapeHtml(supporter.interest || "Belum mengisi minat")}</p>
      </article>
    `).join("");
  }
  ...
  function openFieldNoteModal() {
    ...
    $("#fieldNoteForm").addEventListener("submit", (event) => {
      ...
      closeModal();
      renderAksi();
      showToast("Catatan lapangan tersimpan.");
    });
  }
  ...
  function saveSupporter(event) {
    ...
    saveState();
    renderKolaborasi();
    showToast("Dukungan tersimpan di prototype.");
  }

  function bindArticleCards() {
    $$("[data-article]").forEach((articleNode) => {
      articleNode.addEventListener("click", () => {
        const article = articleSeed[Number(articleNode.dataset.article)];
        openModal(`<span class="tag">${escapeHtml(article.category)}</span><h2>${escapeHtml(article.title)}</h2><p>${escapeHtml(article.body)}</p>`);
      });
    });
  }
  ...
  function bindAiButtons() {
    $$('[data-ai]').forEach((button) => {
      button.addEventListener("click", () => askNipahAi(button.dataset.ai));
    });
  }

  async function askNipahAi(promptText) {
    ...
    try {
      ...
      $(".modal-body").innerHTML = `<h2>Nipah Lestari AI</h2><p>${escapeHtml(aiPayload.answer).replace(/\n/g, "<br>")}</p>`;
    } catch (error) {
      $(".modal-body").innerHTML = `<h2>Nipah Lestari AI</h2><p class="muted">${escapeHtml(error.message || "AI sedang tidak tersedia.")}</p>`;
    }
  }
  ...
  function showToast(message) {
    ...
    showToast.timer = setTimeout(() => toastBox.classList.remove("show"), 2600);
  }
